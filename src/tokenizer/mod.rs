...
/// Data structure for storing a grapheme in u8.
/// 
/// ## Grapheme
type Grapheme = Vec<Vec<u8>>;

/// Regular expression patter for finding word contractions.
/// 
/// ## GPT2 Token Regex
...
/// ```
///
fn tokens(slice: &[u8]) -> Result<Grapheme, crate::error::Error> {
...
/// The function first decodes the bytes into a gpt2 Unicode string.
fn grapheme(ngram: &[u8]) -> Result<Grapheme, crate::error::Error> {
...
/// 
/// ## Validate byte merge
fn validate_merge(this: &BytePair, other: &BytePair) -> bool {
    ...
    fn add_assign(&mut self, pair: &BytePair) {
        ...
        while let Some((index, current)) = cursor.next() {
            if let Some((_, next)) = cursor.peek() {
                if validate_merge(&next, &pair) && (index + 1) == binding.len() {
                    swap(
                        &mut binding[index],
                        &mut [
                            current[0].to_owned(),
                ...
                if validate_merge(&current, &pair) {
                    swap(
                        &mut binding[index],
                        &mut [
...
pub fn encode(text: &[u8]) -> Result<Vec<u16>, crate::error::Error> {
    ...
    while let Some(iteration) = encoder.next() {
        tokens = iteration;
        // println!("[ENCODING]: {:?} {:?}", &String::from_utf16(&tokens).unwrap(), tokens);
    }
    Ok(tokens)
}
